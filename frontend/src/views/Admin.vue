    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="文章管理" name="article">
        ...
        <el-dialog v-model="showEditor" width="70%">
          ...
          <v-md-editor v-model="form.content" height="400px" :autofocus="true" />
      ...
      <el-tab-pane label="轮播图管理" name="banner">
        ...
        <el-table :data="banners" style="width: 100%" class="mt">
          <el-table-column prop="img" label="图片">
            <template #default="scope">
              <img :src="scope.row.img" style="width:120px;height:40px;object-fit:cover;" />
            </template>
          </el-table-column>
        ...
        <el-dialog v-model="showBannerEditor" width="400px">
          ...
          <el-input v-model="bannerForm.link" placeholder="跳转链接（可选）" class="mb" />
...
const form = ref({ title: '', content: '', category_id: '', tags: '', is_featured: false });
...
function submitArticle() {
  if (editId.value) {
    axios.put(`http://localhost:8000/api/articles/${editId.value}`, form.value).then(() => {
...
function addCategory() {
  axios.post('http://localhost:8000/api/categories', { name: newCategory.value }).then(() => {
    newCategory.value = '';
    fetchCategories();
  });
}
...
function saveBanner() {
  if (bannerForm.value.id) {
    axios.put(`http://localhost:8000/api/banners/${bannerForm.value.id}`, bannerForm.value).then(() => {
      showBannerEditor.value = false;
      bannerForm.value = { id: null, img: '', link: '' };
      fetchBanners();
    });
  } else {
    axios.post('http://localhost:8000/api/banners', bannerForm.value).then(() => {
      showBannerEditor.value = false;
      bannerForm.value = { id: null, img: '', link: '' };
      fetchBanners();
    });
