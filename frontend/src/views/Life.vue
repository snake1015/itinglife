  ...
  <div>
    <el-row :gutter="20">
      <el-col :span="18">
        <el-card>
          <div class="flex-between">
            <el-date-picker v-model="selectedDate" type="date" placeholder="选择日期" @change="fetchLogs" />
            <el-button type="primary" @click="showEditor=true">新建日志</el-button>
          </div>
          <el-divider>生活日志</el-divider>
          <el-list>
            <el-list-item v-for="log in logs" :key="log.id" @click="viewLog(log)">
        ...
        <el-dialog v-model="showEditor" width="70%">
          ...
          <v-md-editor v-model="form.content" height="300px" :autofocus="true" @upload-image="handleUploadImage" />
      ...
      <el-col :span="6">
        <el-card>
          <div>多媒体展示：</div>
          <el-image v-for="img in mediaImages" :key="img" :src="img" style="width:100px;height:100px;margin:4px;" fit="cover" />
          <video v-for="vid in mediaVideos" :key="vid" :src="vid" style="width:100px;height:100px;margin:4px;" controls />
        </el-card>
        <el-card class="mt">
          <div>互动式地图（占位）</div>
        </el-card>
...
function fetchLogs() {
  ...
  axios.get('http://localhost:8000/api/articles', { params: { category_id: 2 } }).then(res => {
    logs.value = res.data;
  });
}
...
function submitLog() {
  ...
  axios.post('http://localhost:8000/api/articles', { ...form.value, category_id: 2 }).then(() => {
